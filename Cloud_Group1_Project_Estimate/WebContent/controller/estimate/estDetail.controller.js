...
//			});
			...
			var filter = oEvent.getParameter("arguments").EstPath;
			this.getest(filter);
			
		},
		
		//임시 데이터 전달 필드
		getest : function(filter){
	        ...
	        var url;
			url = "/getestiSet?$filter=PEstno eq '" + filter + "'";
...
//	        url = "/getestiSet";
	        ...
	        var oModel = new sap.ui.model.json.JSONModel({ "estlist": data });
	        this.getView().setModel(oModel , "estlist");
	        
	        
//	        총합 로직으로 최종 합계 금액은 sum에 담기게 된다 
	        var aaa = parseInt(oModel.oData.estlist[0].EstTotal);	// 배열
	        var bbb = oModel.oData.estlist.length;
	        var sum = 0;
	        var i = 0;
	        for (i = 0; i < bbb; i++) {
